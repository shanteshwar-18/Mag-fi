...
/**
 * SensorEngine — ported identically from App 1 (MagFiMapper).
 *
 ...
 * Delivers filtered values via Listener callback on the MAIN thread.
 *
 * Key App 2 difference: NO CSV logging here — only live position updates.
 */
class SensorEngine(private val context: Context) : SensorEventListener {
    ...
    // Sensor availability flags (checked by ViewModel for error reporting)
    var hasAccelerometer = false
        private set
    var hasMagnetometer  = false
        private set

    // Moving average buffers — 9 total (3 axes × 3 sensor types)
    ...
    fun start(listener: Listener) {
        ...
        if (gyro == null) {
            Log.w(TAG, "No gyroscope — will continue without it")
        } else {
